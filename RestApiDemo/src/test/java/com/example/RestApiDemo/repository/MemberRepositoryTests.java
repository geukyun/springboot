...
public class MemberRepositoryTests{
    ...
    public void doBeforeEach(){
        memberRepository.save(Member.builder()
                .name("윤서준").email("SeojunYoon@hanbit.co.kr")
                .age(10).enabled(true).build());
        memberRepository.save(Member.builder()
                .name("윤광철").email("KwancheolYoon@hanbit.co.kr")
                .age(43).enabled(true).build());
        memberRepository.save(Member.builder()
                .name("공미영").email("MiyeongKong@hanbit.co.kr")
                .age(26).enabled(true).build());
        memberRepository.save(Member.builder()
                .name("김도윤").email("DoyunKim@hanbit.co.kr")
                .age(10).enabled(true).build());
    }
    ...
    public void testUserCase1(){
        ...
        // 이름이 '윤서준'이거나 또는 이메일이 'KwancheolYoon@hanbit.co.kr'인 사용자를 조회한 결과 개수가 2인지 검증
        assertThat(memberRepository.findByNameOrEmail("윤서준", "KwancheolYoon@hanbit.co.kr").size()).isEqualTo(2);

        // 이름이 '윤'이라는 글자가 포함된 사용자를 조회한 결과 개수가 3인지 검증
